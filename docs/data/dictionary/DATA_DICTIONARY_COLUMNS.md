# First-Helios — Column-Level Data Dictionary
...
### stores (table: `chain_locations`, ORM class: `Store`)
...
|---|---|---|---|---|---|---|---|
| store_num | VARCHAR | ✗ | Unique store identifier | `OV-ST-a3f7b291` | Format: `{SOURCE}-{CHAIN}-{ID}` | Adapter (Overture, GMAPS, JobSpy) | Set at insert |
| brand_key | VARCHAR | ✓ | **Canonical chain key** — use for all filtering | `starbucks` | Matches `ref_brands.brand_key`; NULL for unrecognized scraped records | Config / Overture adapter | Set at insert |
| chain | VARCHAR | ✗ | Display name from the scraper source (may be inconsistent) | `Starbucks` or `Dutch Bros. Coffee` | Free text — do NOT use for filtering | Scraper source | Set at insert |
| industry | VARCHAR | ✗ | Internal industry key | `coffee_cafe` | One of ~20 keys in `ref_industry_taxonomy.industry_key` | Derived from chain config | Set at insert |
| store_name | VARCHAR | ✗ | Location display name | `Starbucks - Congress Ave` | Free text | Job postings, Google Maps, Overture | Updated weekly |
| address | VARCHAR | ✗ | Physical street address | `123 Congress Ave, Austin, TX 78701` | Formatted address | Overture, Google Maps | Updated weekly |
| lat | FLOAT | ✓ | Latitude (WGS84) | `30.2672` | -90 to 90 | Overture, Google Maps | Updated weekly |
| lng | FLOAT | ✓ | Longitude (WGS84) | `-97.7431` | -180 to 180 | Overture, Google Maps | Updated weekly |
| region | VARCHAR | ✗ | Region key | `austin_tx` | One of `ref_regions.region_key` | Config | Set at insert |
| source_discovery | VARCHAR | ✓ | Which adapter created this record | `overture_local`, `gmaps`, `jobspy` | `alltheplaces`, `overture_local`, `gmaps`, `jobspy` | Adapter name | Set at insert |
| first_seen | DATETIME | ✓ | When first identified | `2025-10-15 14:32:00` | ISO 8601 UTC | Adapter run time | Set at insert |
| last_seen | DATETIME | ✓ | Last observed active | `2026-03-22 10:45:00` | ISO 8601 UTC | Adapter run time | Real-time |
| is_active | BOOLEAN | ✓ | Currently operating? | `true` | true, false | Manual or adapter-detected closure | Updated weekly |

**Notes:**
- `brand_key` is the correct field for all chain filters — `chain` contains raw scraper strings that vary by source
- `lat`/`lng` may be NULL if geocoding failed; back-filled on next scraper run
- Closed stores (`is_active=false`) are retained for historical scoring
...
**Purpose:** Truly-local (non-chain) employer POIs. Map layer + `local_alternatives` scoring component.
**Primary Key:** `id` (auto-increment)
**Unique:** `overture_id`

| Column | Type | Nullable | Description | Example | Valid Values | Source | SLA |
|---|---|---|---|---|---|---|---|
| id | INTEGER | ✗ | Auto-increment PK | `8821` | >= 1 | System | Immutable |
| overture_id | VARCHAR | ✗ | Overture Maps canonical place ID | `08f2a3b...` | Overture UUID | Overture | Set at insert |
| name | VARCHAR | ✗ | Business name | `Epoch Coffee` | Free text | Overture | Set at insert |
| category | VARCHAR | ✓ | Overture category tag | `coffee_shop` | Overture category taxonomy | Overture | Set at insert |
| industry | VARCHAR | ✓ | Internal taxonomy key (mapped from category) | `coffee_cafe` | One of ~20 keys in `ref_industry_taxonomy.industry_key` | `CATEGORY_INDUSTRY_MAP` in overture_adapter.py | Set at insert |
| address | VARCHAR | ✓ | Street address | `221 W North Loop Blvd` | Free text | Overture | Set at insert |
| lat | FLOAT | ✓ | Latitude (WGS84) | `30.3125` | -90 to 90 | Overture | Set at insert |
| lng | FLOAT | ✓ | Longitude (WGS84) | `-97.7312` | -180 to 180 | Overture | Set at insert |
| region | VARCHAR | ✓ | Region key | `austin_tx` | One of `ref_regions.region_key` | Config | Set at insert |
| location_count | INTEGER | ✓ | How many times this exact name appears in Austin data | `1` | >= 1; backfilled by classify_local_employers.py | Script | After classify run |
| source_discovery | VARCHAR | ✓ | Which ingest path created this record | `overture_local` | `overture_local`, `osm` | Adapter | Set at insert |
| confidence | FLOAT | ✓ | Overture data quality score | `0.87` | 0.0–1.0 | Overture | Set at insert |
| upward_mobility | BOOLEAN | ✓ | Is this employer a realistic career step-up from entry-level service work? | `false` | true, false | Derived from category in `UPWARD_MOBILITY_CATEGORIES` | Set at insert |
| is_active | BOOLEAN | ✓ | Currently operating? | `true` | true, false | Overture operating_status | Set at insert |
| first_seen | DATETIME | ✓ | When first ingested | `2026-03-10 12:00:00` | ISO 8601 UTC | System | Set at insert |
| last_seen | DATETIME | ✓ | Last time confirmed present | `2026-03-23 09:00:00` | ISO 8601 UTC | System | Updated on re-ingest |

**Notes:**
- `location_count` is NULL until `scripts/classify_local_employers.py` is run
- Records where `location_count >= 5` are purged by that script (chain-like businesses)
- `upward_mobility = true` records are highlighted in Career Pathfinder employer recommendations
- The `/api/local-employers` and `/api/map-employers` endpoints exclude records with `lat IS NULL`
- Random sampling (`ORDER BY RANDOM() LIMIT N`) is used when no industry filter is set to ensure geographic coverage
...
| 1.1 | 2026-03-24 | Added brand_groups, ref_occupation_aliases; updated to PostgreSQL |
...
**Table:** `brand_groups`
**Schema:** Operational
**Purpose:** Deduplicated employer brand clusters built atomically during `ingest_layer.py` upserts. Drives chain vs local classification (`location_count >= 5` → chain).

| Column | Type | Nullable | Description | Example |
|--------|------|----------|-------------|---------|
| `id` | Integer | ✗ | Auto-increment primary key | 1 |
| `fingerprint` | String | ✗ | Normalized brand name hash for deduplication (lowercase, stripped punctuation/legal suffixes) | `"starbucks"` |
| `canonical_name` | String | ✗ | Clean display name for this brand | `"Starbucks"` |
| `location_count` | Integer | ✗ | Number of Austin POIs sharing this brand fingerprint | 87 |
| `industry` | String | ✓ | Dominant industry key from the brand's POIs | `"coffee_cafe"` |
| `updated_at` | DateTime | ✗ | Timestamp of last upsert | `2026-03-24T10:15:00` |

**Unique constraint:** `fingerprint`
**Classification rule:** `location_count >= 5` → classified as chain (excluded from local_employers map layer)

---

## ref_occupation_aliases

**Table:** `ref_occupation_aliases`
**Schema:** Reference
**Purpose:** Maps Census job-title strings (everyday language) to SOC codes. Powers the Career Pathfinder autocomplete — so users can search "barista" or "data entry" instead of knowing SOC codes.

| Column | Type | Nullable | Description | Example |
|--------|------|----------|-------------|---------|
| `id` | Integer | ✗ | Auto-increment primary key | 1 |
| `alias` | String | ✗ | Census occupation title (lowercase) | `"baristas and coffee makers"` |
| `soc_code` | String | ✗ | SOC code this alias maps to | `"35-3023"` |
| `census_code` | String | ✓ | 3-digit Census occupation code | `"4020"` |
| `industry_restriction` | String | ✓ | If set, alias only applies within this industry context | `"food_service"` |
