...
- 2026-04-21: discovery recall improved for menu-rich first-party sites. `website_scraper.py` now promotes schema.org `Menu.url`, `MenuSection.url`, and `Restaurant.hasMenu` targets into page discovery, and `_discover_pdf_links()` now accepts `.pdf` URLs with query strings by checking the parsed path. Focused regressions in `tests/HeliosDeployment/test_website_scrape_audit_tools.py` pass locally. A targeted 10-site live refresh on under-covered menus improved 5 of 10 sites with net `delta items=+13` and `delta prices=+13`; remaining misses still cluster around price-less or JS-rendered menu pages.
...
| `DISC-03` | Hidden first-party promo or menu pages are still under-discovered on footer-only promos, stable slugs, and scoped first-party sitemap paths | Initial promo-card and footer scoring plus structured menu URL promotion from JSON-LD landed, but bounded slug and sitemap probing are still open | Compare replay manifests before and after, especially `discovery_found_candidates_but_zero_signal`, and add focused replay tests | Partial |
| `DOM-01` | Pages with useful menu structure still collapse into flat blocks and lose item-price relationships | Discovery now reaches more of these pages, but `_extract_text_blocks()` still drops too much heading, sibling, list, and table context before later parsing stages can use it | Work from `content_seen_but_zero_signal` replay sets, add regression tests, and confirm improved item-price or heading-target pairing, especially on pages where page or section counts rise but item and price counts stay flat | Open |
