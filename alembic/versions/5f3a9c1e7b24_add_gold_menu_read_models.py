"""Add the Gold current-menu read model.

Creates the ``gold`` schema and the single ``current_menu`` projection table
(ADR-0006). Gold is rebuildable: downgrade drops the projection and schema and
never touches Bronze, Identity or Menu. FKs reference only lower layers with
``RESTRICT``; no authoritative table references Gold. Execute against disposable
data for review.

Revision ID: 5f3a9c1e7b24
Revises: d83f0a21c592
...
revision: str = "5f3a9c1e7b24"
down_revision: str | Sequence[str] | None = "d83f0a21c592"
...
def downgrade() -> None:
    op.execute("DROP TABLE gold.current_menu")
