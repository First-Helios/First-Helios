"""Add immutable Bronze lookups and Identity batch admission providers.

Only functions are added. No tables, rows, existing functions, or policies are
rewritten. Downgrade removes exactly these additions without CASCADE.

Revision ID: b72e6a90c431
Revises: 91f4c2a7d6e8
...
revision: str = "b72e6a90c431"
down_revision: str | Sequence[str] | None = "91f4c2a7d6e8"
...
def downgrade() -> None:
    for signature in (
        "identity.require_resolved_scopes(bigint[], bigint[], bigint[])",
        "identity.lock_scope_inputs(bigint[], bigint[])",
        "identity.has_pending_lineage(bigint[])",
        "identity.subject_feature_ready(bigint)",
        "identity.scope_dependencies(bigint[])",
        "bronze.evidence_supports_version(bigint, bigint)",
        "bronze.evidence_info(bigint)",
        "bronze.record_version_info(bigint)",
        "bronze.capture_business_key(bigint)",
    ):
